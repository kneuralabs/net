...
function FeedSection() {
  ...
  useEffect(() => {
    // Best-effort live fetch — tries to scrape the LinkedIn company page via a
    // public CORS proxy. Falls back to the last-known value (184) on any error.
    // If you have your own endpoint, swap PROXY + parse logic below.
    let cancelled = false;
    setStatus("loading");
    ...
    const fetchCount = async () => {
      try {
        ...
        // LinkedIn's public HTML embeds the count in og:description and in a
        // visible "X followers" string. Try both patterns.
        const patterns = [
